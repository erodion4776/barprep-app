...
# ---- Pollinations Ad Patterns To Strip ----
AD_PATTERNS = [
    r'pollinations\.ai[^\n]*',
    ...
    r'via pollinations[^\n]*',
    r'\[.*?pollinations.*?\]',
    r'🌸[^\n]*',  # Pollinations sometimes adds flower emoji
]
...
def clean_llm_response(text: str) -> str:
    """
    Removes promotional injections from
    Pollinations AI free tier responses
    ...
    # Remove extra blank lines left by removals
    cleaned = re.sub(r'\n{3,}', '\n\n', cleaned)
...
def local_hash_embedding(text: str) -> List[float]:
    """
    Generates a deterministic 384-dim embedding
    using character-level hashing.
    No external API needed - works on Render free tier.
    """
...
def get_embeddings_batch(texts: List[str]) -> List[List[float]]:
    ...
    1. HuggingFace API (best quality)
    2. Local hash embedding (always works)
    """
    ...
    try:
        ...
        logger.warning(
            f"HuggingFace unavailable: {e}. Using local fallback."
        )
    ...
    for attempt in range(3):
        try:
            ...
            if "429" in str(e) or "too many requests" in error_str:
                ...
                logger.warning(
                    f"YouTube rate limit. Waiting {wait_time}s..."
                )
...
Do NOT include any promotional text, links, or service mentions in your response.
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are an inspiring mentor to future lawyers. "
                ...
                },
                {
                    "role": "user",
                    "content": (
                        "Give a short 2-3 sentence bar exam affirmation. "
                        "Only include the affirmation text, nothing else."
                    )
                }
            ],
            "model": "openai"
